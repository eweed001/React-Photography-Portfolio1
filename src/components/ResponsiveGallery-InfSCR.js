...
class ResponsiveGallery extends Component {
  ...
  loadMoreImages(page) {
    ...
    const perPage = 10;
    const moreItems = this.state.imageArray.slice(
      page * perPage - perPage,
      page * perPage + 1
    );
    //logic for hasMore images: get the length of the array 
    if (this.state.loadedImages.length > 0) {
      console.log("HELOOOOO");
      
      this.setState({ loadedImages: this.state.loadedImages.concat(moreItems) });
